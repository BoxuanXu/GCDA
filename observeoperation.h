...
void write_input_geos(const char* geos_output, const vector<observation>& obs, const datetime & start_date, const datetime& end_date_inter,const datetime& end_date, fileconfig);
			...
			printf("PROCESS %d start handling ensemble-%.3lu...\n", mpi_rank, i);
				...
				sprintf(cmd_str, "cp %s %s/ensemble-%.3lu/", fc.restart_f, fc.DA_dir, i );
                	...
                	/* 
                 	...
                 	write_input_geos(cmd_str, obs, DA_start, DA_end, DA_end, fc);
...
void write_input_geos(const char* geos_output, const vector<observation>& obs, const datetime & start_date, const datetime& end_date_inter,const datetime& end_date,fileconfig fc){
    ifstream geos_conf;
    geos_conf.open(fc.input, ifstream::in);
    ofstream out_conf;
